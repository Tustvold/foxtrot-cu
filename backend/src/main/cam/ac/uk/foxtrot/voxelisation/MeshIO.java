                ...
                if (l.startsWith("f"))
                {
                    ...
                    try
                    {
                        ...
                        while (true)
                        {
                            ...
                            String[] sai = sa[fir].split("/");
                            polygon.add(new Point3d(points.get(Integer.parseInt(sai[0]) - 1)));
                            fir++;
                        }
                        if (polygon.size() < 3)
                        {
                            throw new IOException("Non polygonal input face detected");
                        }
                        Triangulator triangulator = new Triangulator(polygon);
                        triangles.addAll(triangulator.getTriangulatedArrayList());
                        /*
