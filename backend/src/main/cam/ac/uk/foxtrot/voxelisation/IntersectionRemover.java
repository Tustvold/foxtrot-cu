...
public class IntersectionRemover {
    ...
    private Point3dPolygon[] merge(Point3dPolygon polygon1, Point3dPolygon polygon2) {
        ...
        if (!temp1.isEmpty() || touching(polygon1Area,polygon2Area)) { // polygons intersect or touch
    ...
    private ArrayList<Point3d[]> generatePolygons(Area area) {
        ...
        while (!iterator.isDone()) {
            ...
            if(type != PathIterator.SEG_CLOSE) { // reached end of current polygon
            ...
            if (inside(currentExterior,p)) { // is currentExterior inside current p
                currentExterior = p;
            }
        }
